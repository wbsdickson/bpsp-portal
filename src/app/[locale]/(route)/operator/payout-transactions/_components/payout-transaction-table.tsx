...
import usePayoutTransactionTableColumn, {
...
export default function PayoutTransactionTable({
  addTab,
}: {
  ...
  const transactions = usePayoutTransactionStore((s) => s.transactions);
  const invoices = useInvoiceStore((s) => s.invoices);
  const merchants = useMerchantStore((s) => s.merchants);
  const clients = useClientStore((s) => s.clients);

  const rows: PayoutTransaction[] = React.useMemo(() => {
  ...
  const { columns } = usePayoutTransactionTableColumn({ addTab });
