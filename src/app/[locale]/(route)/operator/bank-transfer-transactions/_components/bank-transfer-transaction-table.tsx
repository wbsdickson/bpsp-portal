...
import useBankTransferTransactionTableColumn, {
  type BankTransferTransactionRow,
...
export default function BankTransferTransactionTable({
  addTab,
}: {
  ...
  const transactions = useBankTransferTransactionStore((s) => s.transactions);
  const invoices = useInvoiceStore((s) => s.invoices);
  const merchants = useMerchantStore((s) => s.merchants);
  const clients = useClientStore((s) => s.clients);

  const rows: BankTransferTransactionRow[] = React.useMemo(() => {
  ...
  const { columns } = useBankTransferTransactionTableColumn({ addTab });
