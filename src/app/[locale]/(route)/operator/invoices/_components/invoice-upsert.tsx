  ...
  return (
    <ScrollArea className="h-[calc(100vh-120px)] pr-2">
      <div className="space-y-8">
              ...
              render={({ field }) => (
                <FormItem>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <FormControl>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t("upsert.selectCurrency")} />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
        ...
        <section className="space-y-3">
          <div className="text-sm font-semibold">{t("upsert.remark")}</div>
          <div className="">
            <FormField
              control={form.control}
              name="remark"
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder={t("upsert.remarkPlaceholder")}
                      className="min-h-24"
                    />
                  </FormControl>
                  <FormMessage />
...
export function InvoiceUpsertPage({ invoiceId }: { invoiceId?: string }) {
  ...
  const onSubmit = form.handleSubmit((data: InvoiceUpsertFormValues) => {
    ...
    if (invoiceId) {
      ...
      const path = absoluteUrl(`/${locale}/invoice/${data.invoiceNumber}`);
      addInvoice({
        merchantId: data.merchantId,
        clientId: data.clientId,
        invoiceDate: data.invoiceDate,
    ...
    router.push(`/${locale}/operator/invoices`);
  });
